...
public class ControllerStartWork implements Callback<ResponseModel> {
    public void postData(Context context, Integer applyerId) {
        ((ServiceStartWork) new Retrofit.Builder().baseUrl(AppConstants.BASE_URL).addConverterFactory(GsonConverterFactory.create(new GsonBuilder().setLenient().create())).build().create(ServiceStartWork.class)).startWork(PreferencesUtil.readString(context, PreferencesUtil.KEY_TOKEN, ""), applyerId).enqueue(this);
    }
